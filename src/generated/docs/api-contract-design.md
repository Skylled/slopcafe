...
This follows the shape of [`vector-search-design.md`](/docs/vector-search-design) /
[`source-retention-design.md`](/docs/source-retention-design) / [`byte-exact-publish-design.md`](/docs/byte-exact-publish-design): problem →
...
- **Security schemes** — model the three credential types from §Authentication of
  [`http-api.md`](/docs/http-api) as OpenAPI `securitySchemes` (`awh_` bearer, operator bearer,
...
So OpenAPI is the **precise shape reference**; [`http-api.md`](/docs/http-api) is the **behavioral
...
3. **`docs/README.md`** — note the spec as a second machine-readable artifact
   alongside the prose.
4. **[`agent-knowledge-host-spec-SOLO-v1.md`](/docs/spec-solo)** — the contract surface gains a
...
3. **Consumer adoption — partly done.** ✅ The Dart **CLI** (`cli/`) vendors the
   ...
   ✅ [`http-api.md`](/docs/http-api)'s shape tables re-point at the spec (§9).
...
- **Before `1.0.0`** the contract ran **pre-stable (`0.x`)** while it was still
  ...
  — additive **or** breaking — and the **patch** was docs-only. That phase is
  closed; `1.0.0` froze the contract for real consumers and the strict rules
  above are now the only ones in force.
...
- ~~**MCP `outputSchema`** — phase 4; inputs are already schema'd.~~ ✅ Built
  (Phase 4a — §7).
- **Generating [`http-api.md`](/docs/http-api)'s shape tables** from the spec (vs. linking) —
