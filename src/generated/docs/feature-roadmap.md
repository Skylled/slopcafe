...
*current* contract, see [`http-api.md`](/docs/http-api); this doc is about what's
*next*.

> **Note on links.** Each "Design" link is a repo-relative path to the note's
> source; the docs bundler ([`scripts/build-docs.mjs`](https://github.com/Skylled/slopcafe/blob/main/scripts/build-docs.mjs))
...
**Status:** Deferred — post-V1 · **Design:** [`content-domain-design.md`](/docs/content-domain-design)
...
**Status:** Deferred — post-V1 (gated on multi-domain) · **Design:** [`content-domain-design.md`](/docs/content-domain-design) (serving half) + [issue #3](https://github.com/Skylled/slopcafe/issues/3) (authoring/sanitizer half)
...
**Status:** Proposed · **Design:** [`font-support-design.md`](/docs/font-support-design)
...
**Status:** Partially shipped (data model **and** the agent write path built; classifier agent pending) · **Design:** [`librarian-design.md`](/docs/librarian-design) · vocabulary: `slopcafe-tag-authority` (not yet published; will resolve at `/s/slopcafe-tag-authority` on-platform once it is)
...
- **Issue #43 reshaped the write model — and the fix was none of the obvious
  ...
  *version* — so the operator's approval was attached to something that then
  ...
  [docs/security-model.md](/docs/security-model).
...
- **Wiki-style backlinks / the link graph** ([issue #40](https://github.com/Skylled/slopcafe/issues/40)) — [`backlinks-design.md`](/docs/backlinks-design)
- **Semantic / hybrid search** — [`vector-search-design.md`](/docs/vector-search-design)
- **Source retention + edit-on-source** — [`source-retention-design.md`](/docs/source-retention-design)
- **Document-level CSS (`<style>` blocks)** — [`style-support-design.md`](/docs/style-support-design)
- **Code-first API contract + OpenAPI** — [`api-contract-design.md`](/docs/api-contract-design)
- **Dynamic client registration** (paste-the-URL connect flow) — [`dcr-design.md`](/docs/dcr-design)
- **The Dart CLI** — [`cli-design.md`](https://github.com/Skylled/slopcafe/blob/main/docs/design/cli-design.md) (repo-only, not bundled): the headless counterpart to the MCP connector, and the first real consumer to codegen its client off `openapi.json`.
