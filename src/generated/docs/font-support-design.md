...
[`style-support-design.md`](/docs/style-support-design) and
[`source-retention-design.md`](/docs/source-retention-design): motivation → current state →
...
([`style-support-design.md`](/docs/style-support-design)). Typography is the one axis that
...
This is the same reasoning [`style-support-design.md`](/docs/style-support-design) §4 used
...
| **Font-parser CVEs** (rasterizer bugs) | Same exposure as `data:` fonts — already an accepted browser-0-day-class residual in [`../security-model.md`](/docs/security-model); curated bytes are *lower* risk than arbitrary | Same, plus the bytes are outside our control |
...
- `GET /fonts/:family/:file` in `src/index.ts` (+ its own cache/CORS headers, §6.1);
  ...
  and update the contract ledger/version according to the active window. During
  the current open `3.0.0` window the version stays frozen and this additive
  route belongs on its "Additive since" list; after that window lands, ordinary
  per-change semver resumes and this addition takes a **MINOR** bump. See §14 of
  the API-contract design note.
