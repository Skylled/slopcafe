...
| Revocation is instant and destroys the bytes. | `revokeDocumentCore` in [`src/core.ts`](https://github.com/Skylled/slopcafe/blob/main/src/core.ts): the kill flag flips *before* storage cleanup (unreachable even if the purge is slow), and the purge deletes the render **and** the retained source for every version. |
