...
> Working spec for Phase 2 of [`api-contract-design.md`](/docs/api-contract-design). Generated + adversarially
...
7. Docs-sync (same commit): `docs/http-api.md` gets a `GET /openapi.json` entry; `CLAUDE.md` "Where things live" gets `src/openapi.ts` + `openapi.json`; [`api-contract-design.md`](/docs/api-contract-design) §13 phase 2 → mark built. (Phase 2 is still wire-invisible EXCEPT the new `/openapi.json` route — so the wire-contract docs only gain that one endpoint.)
