 ...
 * see reader-theme.ts (imported above) for the full design comment: why this
 * is safe with no CSP change, the two-theme selection keyed on `doc_kind`,
 * and the dense theme's differences including the sticky-H2 verdict. A
 * Markdown doc is stored as a bare sanitized HTML fragment with no author
 * styling (the sanitizer strips `<style>`), so without this the page renders
 * with the browser's stark, full-width defaults; the theme lives in serving
 * code the sanitizer never touches. It lives in its OWN module rather than
 * here because it has no dependency on core.ts's WASM-backed sanitizer, which
 * lets `readerThemePrefixForDocKind` be unit-tested directly — see
 * test/reader-theme.test.mjs.
 */
...
export async function serveRaw(publicId: string, req: Request, env: Env): Promise<Response> {
  ...
  // served version was written in. `doc_kind` (migration 0019, document-level —
  ...
  // /text derivation; see the design comment above READER_THEME_VARS). WHICH
...
export async function serveVersionRaw(
  ...
  // the DENSE_THEME_CSS comment), so a historical version's reading theme
  // follows the PARENT document's current doc_kind, same as the live /raw path.
  const row = await env.META.prepare(
