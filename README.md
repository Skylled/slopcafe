...
| `*` | `/mcp` | agent (OAuth or awh_) | Streamable HTTP MCP surface — ten typed tools (publish/update/edit/read/list/search docs + set tags/status + load a context pack + mint a publish credential) |
...
Either door yields an `agentId`; the ten MCP tools (`publish_document`, `update_document`, `edit_document`, `set_document_tags`, `set_document_status`, `read_document`, `list_documents`, `search_documents`, `load_context_pack`, `create_publish_credential`) close over it for provenance.
...
src/
  ...
  mcp.ts              MCP server + ten tools; per-request McpServer
...
test/                 pure-unit suites, node --experimental-strip-types, no D1/R2/WASM
                      ...
                      access, session, auth, conditional, integrity, metadata, advisories,
                      contract, openapi. NOTE: mcp-errors.test.mjs and
                      search-ranking.test.mjs exist but are not yet in the npm-script
