...
There's also a no-JS **operator browser console** at **`/admin/console`** (operator session — cookie + CSRF; bare `GET /admin` 302-redirects there). It folds the day-to-day operator work into server-rendered pages so you don't have to `curl` the admin API: browse/search the whole fleet (with `?q=`/`?tag=`/`?slug=` filters and a Public/Private badge per doc), mint/revoke agents, mint/revoke keys, mint bound + unbound OAuth clients (and delete them), edit a document's tags, run the Vectorize + link-graph backfills, and prune expired/long-revoked agent keys (issue #13). It's a thin UI over the same `*Core` functions as the JSON `/admin/*` API (which is unchanged) — see [docs/http-api.md](docs/http-api.md) for the exhaustive route contract.
...
| `POST` | `/admin/keys/prune` | operator | Hard-delete expired/long-revoked `agent_keys` rows (issue #13) |
...
| `*` | `/mcp` | agent (OAuth or awh_) | Streamable HTTP MCP surface — eleven typed tools (publish/update/edit/read/view/list/search docs + set tags/status + load a context pack + mint a publish credential). Optional `?toolset=reader\|author\|full` selects a named preset; `?tools=a,b` remains the exact-list escape hatch. Unknown, empty, or combined selectors return a loud `400`; absent = the default toolset (all but `view_document`, whose embedded viewer is opt-in). This narrows context/affordances, never authorization. |
...
**Pagination** (`GET /d`, `GET /admin/agents`, `GET /admin/agents/:id/keys`, `GET /admin/documents`, and the MCP `list_documents` tool): cursor-based, newest first. Optional `?limit=N` (1..200, HTTP default 50) and `?cursor=<opaque>` query params. The response includes `next_cursor: string | null` — pass it back unchanged on the next call to fetch the next page; `null` means no more pages. Cursors are stable across concurrent writes (insertions or revokes between pages don't skip or duplicate rows). MCP `list_documents` accepts the same `limit` / `cursor` as tool args, with a context-lean default of 10.
...
src/
  ...
  mcp-error-result.ts textError — the code-prefixed structured MCP failure shape
  publish-credential.ts  agent-scoped short-lived awh_ key minting behind create_publish_credential
  ...
  search-ranking.ts   pure ranking helpers shared by the search legs (deprecated-result deboost)
  document-listing.ts the shared DocumentListing projection/decoder + the listing filter SQL every core splices
  ...
  audit.ts            the append-only audit ledger: typed writer (never logs a secret) + read core
  ...
  admin-agents.ts     /admin/agents*, /admin/keys* — agents + credentials
  admin-documents.ts  /admin/documents* + the agent-reachable /d discovery/curation twins
  admin-slugs.ts      /admin/slugs/:slug* — retired-slug redirects + release
  admin-maintenance.ts  /admin/docs/seed, /admin/audit, vectors + links backfill, orphans
  admin-response.ts   the shared JSON error envelope + opaque document 404
...
test/                 pure-unit suites, node --experimental-strip-types, no D1/R2
                      ...
                      advisories, audit, contract, backup-format, openapi, plus mcp-errors,
...
migrations/
  0001_init.sql … 0020_audit_events.sql  20 migrations of schema evolution
                      (oauth clients, source format/retention, metadata, slugs +
                       ...
                       version, per-version OAuth client, audit ledger) — see
                       CLAUDE.md for what each adds
