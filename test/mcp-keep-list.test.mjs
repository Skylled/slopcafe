...
const KEEP = {
  ...
  "update_document": [
    "Identify it by EITHER `public_id` OR `document_slug` — exactly one",
    "it is `document_slug`, NOT `slug`, because `slug` on this tool RENAMES",
    ...
    "METADATA INHERITANCE (where update differs from publish): `title`/`description` are PER-VERSION — omitted = inherited from the prior version unchanged; \"\" clears (title \"\" re-derives from the new content's first <h1>). `tags`/`slug` are DOCUMENT-LEVEL — omitted = left untouched; an explicit value REPLACES (tags) or atomically RENAMES (slug: claims the new, retires the old FOREVER — retired slugs are never freed); \"\" / [] clears.",
    "slug_locked (a PUBLIC document's slug is a reader-facing address, so only the operator may change or clear it; the whole update is refused, content included — re-send without `slug`)",
  ],
  "edit_document": [
    "Identify the doc by EITHER `public_id` OR `document_slug` — exactly one (it is `document_slug`, NOT `slug`, because `slug` here RENAMES).",
    ...
    "`slug_locked` (only the operator may change a PUBLIC doc's slug — re-send without `slug`).",
  ...
  "set_document_tags": [
    ...
    "ERRORS are code-prefixed (\"<code>: <message>\"): not_found (no such LIVE document — a revoked one cannot be re-tagged); invalid_slug; bad_request (both or neither of public_id/document_slug).",
  ],
  "set_document_status": [
    ...
    "ERRORS are code-prefixed (\"<code>: <message>\"): not_found (no such LIVE document); bad_target (`superseded_by` names nothing live, or names this same document); invalid_slug; bad_request (both or neither of public_id/document_slug).",
  ],
  "read_document": [
