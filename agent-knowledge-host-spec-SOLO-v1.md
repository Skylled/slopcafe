...
A **document** is an agent- or operator-authored HTML artifact, versioned, with an unguessable `public_id` and an optional discovery/linking `slug` (§3). Only the sanitized **served** bytes are stored — the pre-sanitized original is **not** retained (convert-and-discard; §5).
...
versions
  ...
  r2_key          text not null         -- R2 object key for the served sanitized bytes (§5)  [key shape <id>/v<n>]
  size_bytes      integer not null      -- for the storage cap (§7)
  sanitizer_v     text not null         -- which sanitizer profile produced the served bytes (§5)
  source_format   text not null default 'html'  -- 'html' | 'markdown' — which input path produced this version
  ...
  -- NOTE: no original_key / sanitized_hash / model column. Originals are not retained (§5); the
  --   writing agent's identity is recorded in R2 customMetadata, not D1; there is no LLM-model column.
...
- **Store** the sanitized bytes **only** — one R2 object per version (`r2_key`, the served form) plus the `sanitizer_v` stamp (so "which docs used a now-vulnerable profile" stays a `WHERE` clause). **The pre-sanitized original is *not* retained** (convert-and-discard). Keeping the original for lazy re-sanitization was deliberately **deferred** to keep the prototype simple while dogfooding; it may be added later (see the re-sanitization note below).
...
**Re-sanitization is deferred — not built (v1).** When the sanitizer profile hardens, **new writes self-heal** (they use the current `sanitizer_v`). Re-healing *already-stored* versions would require the pre-sanitized original — which this build does **not** retain (convert-and-discard, above) — so there is **no** lazy-on-fetch re-sanitization in v1. The `sanitizer_v` stamp still records which profile produced each stored blob, so "which docs used a now-vulnerable profile" remains a `WHERE` clause; *acting* on that answer would require either re-supplying the source bytes or reversing the no-original-retention decision (a conscious prototype simplification — see §8's "if this grows" note). **No link interaction either way:** internal links are stored verbatim as `/s/<slug>` URLs resolved by the public `/s/<slug>` route on every read (§4), so there is nothing to re-resolve — and no write-time link resolution ever existed to repeat.
