...
- **Ammonia's default tag list omits `<section>`, `<main>`, and `<address>`** even though it ships their siblings (`article`/`aside`/`header`/`footer`/`nav`/`hgroup`/`figure`/`figcaption`). Don't assume an HTML5 sectioning element is allowed just because the others are. `make_builder()` re-adds `<section>` via `EXTRA_BLOCK_TAGS` (a plain styled container the publishing contract lists as allowed); `<main>`/`<address>` stay stripped by design (documented as such in `skills/publishing.md`). The positive corpus tests in `lib.rs` (`keeps_section_wrapper_and_its_style`, `keeps_sibling_sectioning_elements`, `strips_main_and_address_but_keeps_children`) pin which sectioning tags survive, so an ammonia upgrade that changes the default set fails loudly.
